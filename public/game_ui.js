...
if (!window.Module.GameManager) {
    ...
    window.Module.GameManager = class JSGameManager {
        ...
        addAnnouncement(text, color) {
            this.announcements.push({text: text, color: color, time: 2.0});
        }
        ...
        update(dt) {
            ...
            this.elapsedTime += dt;
            this.gameSpeed = Math.min(this.maxGameSpeed, this.baseGameSpeed + this.distanceTravelled * 0.015); // increased speed curve a bit

            // Score bleed check
            if (this.buffManager.debuffs[4] > 0) { // SCORE_BLEED
                this.scoreAccumulator = Math.max(0, this.scoreAccumulator - (20 + this.gameSpeed * 0.15 + this.score * 0.05) * dt);
            }

            // Update announcements
            for (let i = this.announcements.length - 1; i >= 0; i--) {
                this.announcements[i].time -= dt;
                if (this.announcements[i].time <= 0) {
            ...
            // Portal trigger
            if (this.score >= 2500 && !this.portalSpawned) { // Flappy Mode at 2500
            ...
            // 5,000 Points spawn milestones
            let currentMilestone = Math.floor(this.score / 5000);
                    ...
                    if (e.type === 1) { // OBSTACLE
                        ...
                        if (e.subtype === 100) { // Level Portal
                            ...
                            if (typeof BUFF_NAMES !== 'undefined') {
                                this.addAnnouncement("+ " + BUFF_NAMES[e.subtype], "#00F0FF");
                            }
                        }
                        e.active = false;
                        audio.playCollect(true);
                        ...
                        if (typeof DEBUFF_NAMES !== 'undefined') {
                            this.addAnnouncement("- " + DEBUFF_NAMES[e.subtype], "#FF5533");
                        }
                        e.active = false;
            ...
            if (this.spawnTimer >= this.nextSpawnInterval) {
                ...
                let diffMult = Math.min(2.0, 1.0 + (this.score / 15000));
                let minSpace = Math.max(0.7, (2.0 - (this.gameSpeed * 0.001)) / diffMult);
                let maxSpace = Math.max(1.3, (3.5 - (this.gameSpeed * 0.001)) / diffMult);
        ...
        spawnRandomEntity() {
            let diffMult = Math.min(2.0, 1.0 + (this.score / 15000));
            let numSpawns = Math.random() > (0.85 / diffMult) ? 2 : 1; 
            let spawnX = 1100;
...
class GameViewport {
    ...
    render(dt) {
        ...
        let themeIndex = Math.floor(this.game.getElapsedTime() / 30) % themes.length;
        ...
        if (this.game.flappyModeActive) {
            ...
            ctx.beginPath();
            ctx.roundRect(-psw * 0.2, psh * 0.8, psw * 1.5, psh * 0.4, 4);
            ctx.fill();
            
            // Cockpit glass
            ctx.fillStyle = "rgba(0, 255, 255, 0.4)";
            ctx.beginPath();
            ctx.roundRect(psw * 0.1, psh * 0.5, psw * 0.8, psh * 0.5, 8);
            ctx.fill();

            // Thrust flame
            ctx.fillStyle = Math.random() > 0.5 ? "#FF5500" : "#FFDD00";
            ctx.beginPath();
            ctx.moveTo(-psw * 0.2, psh * 0.9);
            ctx.lineTo(-psw * 0.8 - Math.random() * psw, psh * 1.0);
            ctx.lineTo(-psw * 0.2, psh * 1.1);
            ctx.fill();
        } else {
        ...
        // Draw announcements
        if (this.game.announcements.length > 0) {
            ctx.textAlign = "center";
            ctx.textBaseline = "middle";
            ctx.font = `bold ${Math.floor(this.scaleH(20))}px "JetBrains Mono"`;
            let baseY = this.scaleY(100);
            this.game.announcements.forEach((ann, idx) => {
                let alpha = Math.min(1.0, ann.time);
                ctx.fillStyle = `rgba(0, 0, 0, ${alpha * 0.7})`;
                ctx.fillText(ann.text, this.scaleX(px + pw / 2) + 2, baseY - (idx * 30) + 2);
                ctx.fillStyle = ann.color;
                ctx.globalAlpha = alpha;
                ctx.fillText(ann.text, this.scaleX(px + pw / 2), baseY - (idx * 30));
            });
            ctx.globalAlpha = 1.0;
        }
